...
def config_to_onboard_request(
    ...
    Args:
        config: Customer configuration from storage
        environment: Target environment for deployment

    Returns:
        CustomerOnboardRequest for the Pulumi deployment
    """
    return CustomerOnboardRequest(
        customer_name=config.customer_id,
...
async def run_deployment(
    ...
    try:
        ...
        # Convert config to onboard request for deployment settings
...
async def deploy(
    ...
    Args:
        ...
        request: Deployment request with environment
    ...
    Raises:
        HTTPException: 404 if customer config not found
        HTTPException: 409 if deployment already in progress
    """
...
async def get_deployment_status(
    ...
    # Check Pulumi for updated status if in progress or destroying
    if (
        deployment.status
        in (
...
async def list_customer_deployments(customer_id: str) -> list[CustomerDeployment]:
    ...
    Returns:
        List of deployments for the customer
    """
...
async def destroy(
    ...
    Args:
        ...
        request: Destroy request with confirmation
        background_tasks: FastAPI background tasks
    ...
    Raises:
        HTTPException: 404 if deployment not found
        HTTPException: 409 if operation already in progress
